...
class MinecraftStream(object):
    ...
    def __getattribute__(self, name):
        if name not in ['readjson', '_original_stream']:
            return getattr(object.__getattribute__(self, '_original_stream'),
                           name)
        else:
...
class MinecraftJsonApi(object):
    ...
    def __createkey(self, method):
        ...
        return sha256('{username}{method}{password}{salt}'.format(
            ...
            salt=self.salt).encode()).hexdigest()
    ...
    def __createURL(self, method, args):
        ...
        return self.__basic_url.format(host=self.host,
                                       port=self.port,
                                       query=urlencode([
                                           ('method', method),
    ...
    def __createStreamURL(self, source):
        ...
        return self.__subscribe_url.format(query=urlencode([
            ('source', source),
            ('key', key),
        ])).encode()
    ...
    def __createMultiCallURL(self, methodlist, arglist):
        ...
        return self.__multi_url.format(host=self.host,
                                       port=self.port,
                                       query=urlencode([
                                           ('method', methodlist),
                                           ('args', arglist),
...
if __name__ == '__main__':
    ...
    api = MinecraftJsonApi(host=params['host'],
                           port=params['port'],
